...
export const VoiceInputCapture = ({ onSave, initialText = "", showWaveform = true, showInterimTranscript = true, customWaveformColor, placeholder = "Press Record button to start the dictation, or type here...", // Updated default placeholder
disabled = false, silenceTimeout = DEFAULT_VIC_SILENCE_TIMEOUT, initialSpeechTimeout = DEFAULT_VIC_INITIAL_SPEECH_TIMEOUT, showVersionInfo = true, textDisplayClassName, interimTranscriptClassName, recordButtonClassName, }) => {
    const [recordingState, setRecordingState] = useState("idle");
    const [interimTranscript, setInterimTranscript] = useState("");
    const [finalTranscript, setFinalTranscript] = useState(initialText);
    const [audioDataForWaveform, setAudioDataForWaveform] = useState(null);
    ...
    const handleFinalTranscriptSegment = useCallback((segment) => {
    ...
    const handleRecordingStop = useCallback(async (audioBlob, audioUrl) => {
    ...
    const handleAudioData = useCallback((dataArray) => {
        if (showWaveform) {
    ...
    const toggleRecording = async () => {
        ...
        if (isRecordingOrListening) {
            ...
            try {
                ...
                handleError(e.message || "Failed to start recording.");
                setRecordingState("idle");
            }
        }
    };
    const handleTextChangeFromEditor = (newText) => {
    ...
    const getButtonIcon = () => {
        ...
        if (isRecordingOrListening)
            return _jsx(StopCircle, { className: "w-4 h-4" });
        return _jsx(Mic, { className: "w-4 h-4" });
    };
    const getButtonText = () => {
        if (recordingState === "error")
            return "Retry";
    ...
    return (_jsxs("div", { className: cn("relative p-3 sm:p-4 border rounded-lg shadow-sm bg-card w-full max-w-2xl mx-auto space-y-3", { "opacity-75 cursor-not-allowed": disabled }), children: [showVersionInfo && componentVersion && (_jsxs("div", { className: "absolute top-2 right-2 flex items-center space-x-1 text-xs text-muted-foreground", children: [_jsx(Info, { className: "w-3 h-3" }), _jsxs("span", { children: ["v", componentVersion] })] })), _jsx("div", { className: "flex flex-col sm:flex-row items-start sm:items-center space-y-2 sm:space-y-0 sm:space-x-2 pt-4", children: _jsx("div", { className: "flex-grow w-full", children: _jsx(EditableTextDisplay, { initialText: finalTranscript, onTextChange: handleTextChangeFromEditor, placeholder: placeholder, className: "w-full", textDisplayClassName: textDisplayClassName, isEditing: isEditing, onEditing: () => {
                            setIsEditing(true);
                        } }) }) }), _jsxs("div", { className: "flex flex-row items-center justify-between", children: [_jsxs(Button, { onClick: recordingState === 'error' ? handleRetryError : toggleRecording, disabled: disabled && recordingState !== 'error', className: cn("flex-shrink-0 w-full sm:w-auto", recordButtonClassName, recordingState === 'error' ? "bg-yellow-500 hover:bg-yellow-600 text-white" : ""), "aria-label": getButtonText(), children: [getButtonIcon(), _jsx("span", { className: "ml-2", children: getButtonText() })] }), isEditing && (_jsxs(Button, { onClick: handleManualSave, className: "flex-shrink-0 w-full sm:w-auto", children: [_jsx(Save, { className: "w-4 h-4 mr-2" }), "Save Text"] }))] }), recordingState === "error" && errorDetails && (_jsxs("div", { className: "flex items-center p-2 text-sm text-destructive-foreground bg-destructive rounded-md", children: [_jsx(AlertTriangle, { className: "w-4 h-4 mr-2 flex-shrink-0" }), " ", _jsxs("span", { children: ["Error: ", errorDetails] })] })), showInterimTranscript && isRecordingOrListening && interimTranscript && (_jsxs("div", { className: cn("p-2 text-sm text-muted-foreground bg-muted/30 rounded-md min-h-[2.5rem] italic", // Default base styles
                interimTranscriptClassName // Custom styles passed as prop
                ), children: ["Live: ", interimTranscript] })), showWaveform && isRecordingOrListening && (_jsx(WaveformDisplay, { audioData: audioDataForWaveform, color: customWaveformColor, className: "w-full h-16" })), showWaveform && recordingState === "idle" && !errorDetails && (_jsx(WaveformDisplay, { audioData: null, color: customWaveformColor, className: "w-full h-16" }))] }));
};
